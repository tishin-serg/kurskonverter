...
func TestP2PCountryBeforePrice(t *testing.T) {
	...
	cheap := base
	cheap.OfferID, cheap.Price, cheap.AllowedCountries = "restricted", d("50"), []string{"GEO", "AFG"}
	for _, tc := range []struct{ country, want string }{{"RUS", "open"}, {"", "open"}, {"GEO", "restricted"}, {"US", "open"}} {
