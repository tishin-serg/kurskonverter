...
Главное меню (`/start` или `/menu`): «Получить BTC», «USD на TBC», «Настройки». Для быстрой работы можно сразу отправить `0.01 BTC`, `5000 руб` или `100 USD`. `/cancel` отменяет ожидаемый ввод. Меню обновляется в одной панели, результаты сохраняются отдельными сообщениями.

В личных настройках доступны курс BTC/RUB, минимум сделок продавца, минимальный процент успешных операций, банки Bybit/Wallet и разрешение резервных комиссий. Эти параметры не меняют настройки других пользователей. После 30 минут ожидания ввод отменяется; устаревшие кнопки не перезаписывают новые значения.

`100 USD` рассчитывает стоимость получения долларов на TBC Bank через RUB → USDT → USD в Bybit. Для каждой стороны выбирается одна заявка, покрывающая весь этап; учитываются лимиты, резерв, рейтинг, точность и свежесть. USD не приравнивается к USDT. Возможная комиссия банка за зачисление не входит в оценку. Проверка API без сделок: `bot check-usd any` (`any` снимает только фильтр рублёвого банка в диагностике).

`/rate 7000000` сохраняет личный курс в рублях за 1 BTC. Затем `5000 руб`, `5000 RUB` или `5000₽` задаёт сумму для отправки в BTC по этому курсу. BTC округляется вверх до сатоши. Бот показывает фактические расходы по рыночным маршрутам отдельно от заданного эквивалента. Ввод `0.01 BTC` работает без личного курса.

Курс доступен через `/settings` → «Курс BTC/RUB». Кнопка «Обновить» сохраняет исходную рублёвую сумму и использует текущий личный курс; «Подробнее» показывает сохранённый расчёт с прежним курсом. В результатах BestChange указано имя выбранного обменника.
